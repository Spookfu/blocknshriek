# Debug
execute if score .functionDebug debug matches 1 run tellraw @a [{"text":"[DEBUG]","color":"blue","bold":true},{"text":" Running function hub/maps/crypt.mcfunction","color":"white","bold":false}]

kill @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{map:1b}}}}]
clear @s[nbt=!{Inventory:[{id:"minecraft:white_dye",Slot:4b}]}] white_dye[custom_data={map:1b}]

playsound minecraft:block.trial_spawner.ambient_ominous master @s[tag=!.mapConsideringBase] ~ ~ ~ 1 0.1 1
tag @s add .mapConsideringCrypt
...
item replace entity @s armor.head with minecraft:oak_button[minecraft:equippable={slot:"head",camera_overlay:"minecraft:text/cryptmap"}]
